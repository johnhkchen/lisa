...
A machine is on one of three channels: canary takes the newest release, nightly takes the newest release once it has aged past the soak window, and stable takes the newest release that is not a prerelease.

Where that channel is written depends on how lisa got onto the box. A Homebrew or apt box reads it off the package — the formula name, or the suite word in the sources line — and upgrade hands the move to brew or apt-get; --channel there switches packages. Every other box keeps it in a per-user config file, and one that has never chosen is treated as stable.

--tag is the way back, and it is not the same everywhere: on apt it is a real rollback from the pool, and on Homebrew it is lisa's own installer writing into ~/.local/bin, because brew switch is gone.
