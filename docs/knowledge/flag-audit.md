...
| `flag:lisa/spend:--path` | Choose the project to fall back to | working default | Default is `.`, the current folder; used only when the desk-wide machine list can't be learned. | `without_rail_on_path_it_falls_back_to_this_project_and_says_so` | — |
| `flag:lisa/spend:--reach-timeout-secs` | Wait longer than the default before calling a machine unreachable | working default | Default is 6 seconds, matching `rail desk`'s own bound for asking a host something. | `a_host_that_never_answers_is_abandoned_at_its_timeout_not_hung_on_forever` | — |
| `flag:lisa/spend:--guard` | Let this board stop its own loop when it is low priority and over its allowance | working default | Default is off, so a bare `lisa spend` only ever reports; asking for `--guard` is what lets it also act, and only on this board itself (T-072-01-02). | `a_low_priority_board_over_its_allowance_stops_and_tells_rail` | — |
...
| `config:scheduling.priority` | This board's own priority for `lisa spend --guard` | working default | Default is `medium`; a board that never configures this is never the one `--guard` stops (T-072-01-02). | `a_board_with_no_configured_priority_is_never_stopped` | — |
| `config:scheduling.weekly_token_allowance` | The token budget `lisa spend --guard` compares this week's spend against | working default | Default is unset, so `--guard` has nothing to act on and never acts (T-072-01-02). | `no_configured_allowance_leaves_the_guard_inert_however_much_was_spent` | — |
